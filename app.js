...
mongoUtil.connectToServer(function(err) {
  ...
  console.log(
    "connected to mongodb. Open app url in browser to authenticate with facebook " +
    ...
    // user is being redirected with code
      ...
      fbApi.authorize(
        ...
        function(err) {
          ...
          fbApi.setAccessToken(process.env.FB_PAGE_LONG_TOKEN);
...
let shareQueueItem = async function() {
  ...
  if (item) {
    ...
    console.log(
      `Sharing submission, title: ${item.payload.title}, domain: ${item.payload.domain}`
    );
...
let postToFb = function(item) {
  ...
  fbApi.post(`/${process.env.FB_PAGE_ID}/feed`, fbPost, function(err, res) {
    if (err) {
      console.dir(err);
    }
  });
...
let updateFromReddit = function(req, res, next) {
  ...
  submissionPollster.getHotPosts().then(function(result) {
...
function stripPersonalTitle(s) {
  if (s.title.includes(" I ") || s.title.includes("I've")) {
    s.title = "";
  }
}
...
function isSubmissionOK(submission) {
  ...
  if (submission.is_nsfw || submission.selftext) {
    return false;
  }
